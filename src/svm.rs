...
use solana_sdk::{rent::Rent, rent_collector::RENT_EXEMPT_RENT_EPOCH};
...
fn load_accounts_from_dump(svm: &mut LiteSVM, dump_file_path: &str) -> Result<u64> {
    // Read the dump file
    ...
    let slot = dump_data["slot"].as_u64().unwrap_or(200_000_000); // Default slot if not found
    ...
    for (i, account_data) in accounts.iter().enumerate() {
        ...
        let data_bytes = if let Some(data_array) = account_data["data"].as_array() {
            // Handle data as array of numbers
            data_array
                .iter()
                .map(|v| v.as_u64().unwrap_or(0) as u8)
                .collect::<Vec<u8>>()
        } else if let Some(data_str) = account_data["data"].as_str() {
            // Handle data as base64 string
            base64::Engine::decode(&base64::engine::general_purpose::STANDARD, data_str).map_err(
                |e| anyhow::anyhow!("Failed to decode base64 data for account {}: {}", i, e),
            )?
        } else {
...
pub fn dump_pool_accounts(addresses: Vec<Pubkey>, rpc_url: &str, output_file: &str) -> Result<()> {
    ...
    println!("Found {} accounts at slot {}", accounts.len(), slot);
